...
class OPTIC_dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.pseudo_label_list[item].endswith("tif"):
            self.pseudo_label_list[item] = self.pseudo_label_list[item].replace(
                ".tif", "-{}.tif".format(1)
            )
        img_file = os.path.join(self.root, self.img_list[item])
        label_file = os.path.join(self.root, self.label_list[item])
        pseudo_label_file = os.path.join(self.root, self.pseudo_label_list[item])
        img = Image.open(img_file)
        label = Image.open(label_file).convert("L")
        pseudo_label = Image.open(pseudo_label_file).convert("L")
        ...
        pseudo_label_npy = np.array(pseudo_label)
        mask = np.zeros_like(label_npy)
        pseudo_mask = np.zeros_like(pseudo_label_npy)
        mask[label_npy < 255] = 1
        mask[label_npy == 0] = 2
        pseudo_mask[pseudo_label_npy < 255] = 1
        pseudo_mask[pseudo_label_npy == 0] = 2
