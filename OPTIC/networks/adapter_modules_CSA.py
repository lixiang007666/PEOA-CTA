...
class CrossAttention(nn.Module):
    def __init__(self, d_model, n_heads, chunk_size=128):
        super().__init__()
    ...
    def forward(self, query, key, value, mask=None):
        ...
        k = (
            self.k_proj(key)
            .reshape(B, N_kv, self.n_heads, self.head_dim)
            .permute(0, 2, 1, 3)
        )
        v = (
            self.v_proj(value)
        ...
        for i in range(0, N_q, self.chunk_size):
            q_end_idx = min(i + self.chunk_size, N_q)
            q_chunk = q[:, :, i:q_end_idx]

            # 对k和v也进行分块处理
            attn = []
            for j in range(0, N_kv, self.chunk_size):
                k_end_idx = min(j + self.chunk_size, N_kv)
                k_chunk = k[:, :, j:k_end_idx]
                v_chunk = v[:, :, j:k_end_idx]

                # 计算当前块的注意力分数
                curr_attn = (
                    torch.matmul(q_chunk, k_chunk.transpose(-2, -1)) * self.scale
                )

                if mask is not None:
                    curr_mask = (
                        mask[:, i:q_end_idx, j:k_end_idx]
                        if mask.dim() == 3
                        else mask[:, None, i:q_end_idx, j:k_end_idx]
                    )
                    curr_attn = curr_attn.masked_fill(curr_mask == 0, float("-inf"))

                attn.append(curr_attn)

                # 主动清理不需要的张量
                del k_chunk, v_chunk, curr_attn
                torch.cuda.empty_cache()

            # 在k维度上连接所有块并计算softmax
            attn = torch.cat(attn, dim=-1).softmax(dim=-1)
            ...
            for j in range(0, N_kv, self.chunk_size):
                k_end_idx = min(j + self.chunk_size, N_kv)
                v_chunk = v[:, :, j:k_end_idx]
                out_chunk += torch.matmul(attn[:, :, :, j:k_end_idx], v_chunk)

                del v_chunk
                torch.cuda.empty_cache()

            out[:, :, i:q_end_idx] = out_chunk
            del attn, out_chunk
            torch.cuda.empty_cache()
...
class Extractor(nn.Module):
    def __init__(
        ...
        #                          n_points=n_points)
        #########################################################
        self.attn = CrossAttention(d_model=dim, n_heads=num_heads)
    ...
    def forward(
        ...
        def _inner_forward(query, feat):
            ...
            attn = self.attn(
                self.query_norm(query), self.feat_norm(feat), self.feat_norm(feat)
            )
            query = query + attn
...
class InteractionBlockPrompt(nn.Module):
    def __init__(
        ...
        if self.next_dim is not None:
            self.up_layer1 = nn.Conv2d(
                dim, self.next_dim, kernel_size=3, stride=stride, padding=1, bias=False
            )
            self.up_layer2 = nn.Conv2d(
                dim, self.next_dim, kernel_size=3, stride=stride, padding=1, bias=False
            )
            self.up_layer3 = nn.Conv2d(
                dim, self.next_dim, kernel_size=3, stride=stride, padding=1, bias=False
            )
    ...
    def deform_inputs(self, hw_shape, device):
        ...
        spatial_shapes = torch.as_tensor(
            [(h // 4, w // 4)], dtype=torch.long, device=device
        )
        level_start_index = torch.cat(
            (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1])
        )
        reference_points = get_reference_points(
            [(h // 2, w // 2), (h // 4, w // 4), (h // 8, w // 8)], device
        )
    ...
    def forward(self, x, c1, c2, c3, stage, hw_shape, backbone="swin", vgg_layers=None):
        ...
        if backbone == "swin":
            x_stage, hw_shape_stage, out_stage, out_hw_shape_stage = stage(
                x_injector, hw_shape
            )
            x_injector = (
                x_injector.view(bs, channel, dim).transpose(1, 2).view(bs, dim, H, W)
            )
            return (
                x_injector,
                c1,
            ...
            x_injector = x_injector.permute(0, 2, 1).view(
                x_injector.shape[0],
                x_injector.shape[2],
                int(x_injector.shape[1] ** 0.5),
                int(x_injector.shape[1] ** 0.5),
            )
            x_stage = stage(x_injector)
            x_injector = (
                x_injector.view(bs, channel, dim).transpose(1, 2).view(bs, dim, H, W)
            )
            return x_injector, c1, c2, c3, x_stage
