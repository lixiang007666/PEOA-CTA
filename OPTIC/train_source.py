...
class TrainSource:
    def __init__(self, config):
        ...
        self.model_path = os.path.join(
            config.path_save_model, config.Source_Dataset
        )  # Save Model
        ...
        sr_img_list, sr_label_list = convert_labeled_list(
            config.dataset_root, source_train_csv
        )
        ...
        self.image_save_path = os.path.join(
            config.path_save_output, "segmentation_results"
        )
    ...
    def build_model(self):
        self.model = ResUnet(
            resnet=self.backbone, num_classes=self.out_ch, pretrained=True
        ).to(self.device)
    ...
    def run(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, data in enumerate(self.source_train_loader):
                x, y = data["data"], data["mask"]
                x = (
                    torch.from_numpy(normalize_image_to_0_1(x))
            ...
            if best_loss > loss_meter.value()[0]:
                ...
                torch.save(
                    ...
                    self.model_path + "/" + "pretrain-Res_Unet.pth",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr_scheduler", type=str, default="Epoch", help="Cosine/Step/Epoch"
    )  # choose the decrease strategy of lr
    ...
    parser.add_argument(
        "--path_save_output", type=str, default="./output/"
