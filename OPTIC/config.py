...
def GDL_loss(pred, label):
    smooth = 1.0
    weight = 1.0 - torch.sum(label, dim=(0, 2, 3)) / torch.sum(label)
    ...
    score = 1.0 - (2.0 * (intersection + smooth) / (union + smooth))
    return score
...
def dice_coeff(pred, label):
    ...
    score = 1 - ((2.0 * intersection + smooth) / (m1.sum() + m2.sum() + smooth))
    return score
...
def jaccard_loss(pred, label):
    ...
    m2 = torch.abs(1 - m2)
    score = 1 - (
        (torch.min(m1, m2).sum() + smooth) / (torch.max(m1, m2).sum() + smooth)
    )
    return score
...
def bce_loss(pred, label):
    ...
    # score = torch.nn.CELoss()(pred, label)
    return score
...
class EpochLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        return [self.lr * pow((1.0 - self.last_epoch / self.epochs), self.gamma)]
