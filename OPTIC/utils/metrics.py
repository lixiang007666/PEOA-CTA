...
def asd_compute(test, reference):
    ...
    for batch in range(batch_size):
        disc_asd, cup_asd = avg_surface_distance(
            test=test[batch][0], reference=reference[batch][0]
...
def hd_compute(test, reference):
    ...
    for batch in range(batch_size):
        disc_hd, cup_hd = hausdorff_distance_95(
            test=test[batch][0], reference=reference[batch][0]
...
def dice_metric(pred, label):
    ...
    for batch in range(batch_size):
        ...
        disc_dice = (2 * disc_intersection + smooth) / (
            pred[batch][0].sum() + label[batch][0].sum() + smooth
        )
        cup_intersection = (pred[batch][-1] * label[batch][-1]).sum()
        cup_dice = (2 * cup_intersection + smooth) / (
            pred[batch][-1].sum() + label[batch][-1].sum() + smooth
        )
